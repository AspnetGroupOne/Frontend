...
const BookingsTable = () => {
  ...
  useEffect(() => {
    fetch('https://localhost:7044/api/booking')
...
// AI-genererad kod: Filtreringslogiken byggdes för att kombinera flera filter samtidigt
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://localhost:7044/api/booking/${id}`, {
        method: 'DELETE'
      });
        ...
        <tbody>
          {bookingsToShow.map((booking) => (
            <tr key={booking.id}>
              ...
              <td>{booking.name}</td>
              <td>{booking.event}</td>
              <td><span className="tag">{booking.ticketCategory}</span></td>
              <td>${booking.price}</td>
  ...
  <button className="edit-button" onClick={() => setEditingBooking(booking)}>Edit</button>
